...
"""
This code implements the Neural Operator class using the precomputed Vandermonde matrices.
"""
...
class SpectralConv2d_fast(nn.Module):
    ...
    def forward(self, x, vfwd, vinv):
        ...
        x = torch.bmm(vinv, x_ft)
        x = x.permute(0, 2, 1)

        return x.real

class PreVNO2D(nn.Module):
    def __init__(self, modes1, modes2, width):
        super(PreVNO2D, self).__init__()
    ...
    def forward(self, x, vfwd, vinv):
        ...
        x1 = self.conv0(x, vfwd, vinv)
        x2 = self.w0(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv1(x, vfwd, vinv)
        x2 = self.w1(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv2(x, vfwd, vinv)
        x2 = self.w2(x)
        x = x1 + x2
        x = F.gelu(x)

        x1 = self.conv3(x, vfwd, vinv)
        x2 = self.w3(x)
        x = x1 + x2
