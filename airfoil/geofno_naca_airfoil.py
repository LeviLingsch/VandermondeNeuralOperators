...
"""
This code was used to benchmark the capabilities of the GeoFNO for the airfoil problem. The foundation for this code can 
be found at the github page here: https://github.com/neuraloperator/Geo-FNO. The associated paper from Li et al., Fourier Neural Operator
with Learned Deformations for PDEs on General Geometries, can be found here: https://arxiv.org/abs/2207.05209.
"""
...
file_path = '/cluster/scratch/llingsch/naca_airfoil/'
...
step_size = configs['scheduler_step']
...
for ep in range(epochs):
    ...
    for input, output in train_loader:
        ...
        optimizer.zero_grad()
        im = model(input, iphi=model_iphi)
        samples_xi = model_iphi(samples_x)

        loss_data = myloss(im.view(batch_size, -1), output.view(batch_size, -1))
        loss_reg = myloss(samples_xi, samples_x)
        loss = loss_data + 0.100 * loss_reg
        loss.backward()
    ...
    test_l2 = 0.0
    with torch.no_grad():
        median_error = torch.zeros(ntest)
        iter=0
        for input, output  in test_loader:
            ...
            im = model(input, iphi=model_iphi)
            test_l2 += myloss(im.view(batch_size, -1), output.view(batch_size, -1)).item()
            median_error[iter] = percentage_difference(output, im)
            iter+=1
    ...
    print(ep, t2 - t1, train_l2, train_reg, test_l2, torch.median(median_error).item())
