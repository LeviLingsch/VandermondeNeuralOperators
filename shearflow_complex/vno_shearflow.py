...
from VNO2DFixed import VNO2DFixed
...
def main(configs):
    ...
    # transformer
    ##############
    transformer = VFT2D(sparse_x, y_pos, modes, modes, device)
    ##############
    # model
    ##############
    # initialize model
    if load_model:
        ...
        model = VNO2DFixed(modes, modes, width, transformer, sparse_x).to(device)
    ...
    if save_model:
        print(f"model: VNO2d_complex_{epochs}_{test_l1_loss/ntest:.4f}")
        torch.save(model, f'/VNO2d_complex_{epochs}_{real_relative_error / ntest:.4f}')
    ...
    with torch.no_grad():
        ...
        for xx, yy in test_loader:
            ...
            print(l1_relative_error_real, l1_relative_error_imag)
            iter+=1
